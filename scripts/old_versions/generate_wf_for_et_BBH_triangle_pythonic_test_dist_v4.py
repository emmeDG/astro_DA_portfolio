...
if __name__=="__main__":
    ...
    if case == 'TERZ': file_list       = ['../sens_curves/ET_ALL_NB.txt', '../sens_curves/ET_TERZ_10th_perc_1year.txt', '../sens_curves/ET_TERZ_50th_perc_1year.txt', '../sens_curves/ET_TERZ_90th_perc_1year.txt']
    elif case == 'SOS': file_list      = ['../sens_curves/ET_ALL_NB.txt', '../sens_curves/ET_P2_10th_perc_1year.txt', '../sens_curves/ET_P2_50th_perc_1year.txt', '../sens_curves/ET_P2_90th_perc_1year.txt']
    ...
    if source_type == 'BNS':
        ...
        idx_tot = np.append(idx_z_base,np.array(idxs_rdn, dtype = 'int')[:,0])
        ...
        idx_tot = np.append(idx_z_base1,idx_z_base2)
        idx_tot = np.append(idx_tot,idx_z_base3)
        idx_tot = np.append(idx_tot,idx_z_base4)
    ...
    for m,sel in enumerate(idx_tot):
            ...
            if config == 'TRIANGLE':
                    ...
                    IFOs     = ['ET1', 'ET2', 'ET3']
                    armlegth = 10000
                    orient   = [[0 ,60*np.pi/180], [300*np.pi/180, 240*np.pi/180], [0, -60*np.pi/180]]
                    nIFOs    = 3
                    
                    if case == 'SOS':
    
                        coords = [0.705851, 0.164917]
                        alt    = 250
                
                    elif case == 'TERZ':
    
                        coords   = [0.885493, 0.103264]
                        alt      = 0
                    ...
                    for i, det in enumerate(IFOs):
                        ...
                        add_detector_on_earth(name = det, longitude = coords[0], latitude = coords[1], height = alt, xlength=armlegth, ylength = armlegth, yangle = orient[i][0], xangle = orient[i][1])
                    ...
                    add_detector_on_earth(name = 'ET1', longitude = 0.705851, latitude = 0.164917, height = 300, xlength=armlegth, ylength = armlegth, yangle = 0)
                    det_ET1.append(Detector('ET1'))
                    add_detector_on_earth(name = 'ET2', longitude = 0.885493, latitude = 0.103264, height = 0, xlength=armlegth, ylength = armlegth, yangle = np.pi/2)
            ...
            for fi, fl in enumerate(file_list):
                ...
                for i in range(len(IFOs)):
                    ...
                    ts_noise_ET1.append(noise_from_sqrt_psd(freq, ETD, fs=fs, dur=dur, seed=i))
                    print('    '+IFOs[i]+' DONE.')
