...
if __name__=="__main__":
    ...
    if len(idx_tot) > 0:
        process_event_chunk([0])
        save_results()

    pending_positions = np.arange(1, len(idx_tot), dtype=int)
    max_workers = min(os.cpu_count() or 1, len(idx_tot)) if len(idx_tot) > 0 else 1

    if len(pending_positions) > 0:
        if max_workers <= 1:
            process_event_chunk(pending_positions)
            save_results()
        else:
