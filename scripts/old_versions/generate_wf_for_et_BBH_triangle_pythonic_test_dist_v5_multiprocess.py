...
def init_process_worker(worker_state):
    global _WORKER_STATE
    _WORKER_STATE = worker_state

def process_event_chunk(event_positions):
    state = _WORKER_STATE
    idx_tot = state['idx_tot']
    events = state['events']
    ...
    coords = state['coords']
    alt = state['alt']
    armlegth = state['armlegth']
    orient = state['orient']
    file_list = state['file_list']
    plots = state['plots']
    cumulative = state['cumulative']
    f_cut = state['f_cut']
    f_start = state['f_start']
    source_apx = state['source_apx']
    source_fs = state['source_fs']
    source_f_lower = state['source_f_lower']
    loss_reference_nominal = state['loss_reference_nominal']

    nominal_chunk    = np.zeros(len(event_positions))
    deci_chunk       = np.zeros(len(event_positions))
    cinqui_chunk     = np.zeros(len(event_positions))
    nova_chunk       = np.zeros(len(event_positions))
    deci_loss_chunk  = np.zeros(len(event_positions))
    cinqui_loss_chunk= np.zeros(len(event_positions))
    nova_loss_chunk  = np.zeros(len(event_positions))
    time_to_merge_chunk = np.zeros(len(event_positions))
    time_to_m_SNR_chunk = np.zeros(len(event_positions))
    time_tot_chunk      = np.zeros(len(event_positions))

    for local_idx, m in enumerate(event_positions):
        sel = idx_tot[m]

        end_time        = events['tGPS'][sel]
        declination     = events['dec'][sel]
        right_ascension = events['ra'][sel]
        polarization    = events['psi'][sel]
        ...
        hp, hc = get_td_waveform(approximant=source_apx,
        ...
        iidx_cut = iidx[0] - int((f.sample_times[-1] - f.sample_times[idx_f_cut])*source_fs)
        iidx_st = iidx[0] - int((f.sample_times[-1] - f.sample_times[idx_f_start])*source_fs)
        ...
        for fi, fl in enumerate(file_list[0:3]):
            ...
            for i in range(len(IFOs)):
                if config == 'TRIANGLE':
                    ts_noise_ET1[i] = noise_from_sqrt_psd(freq, ETD, fs=source_fs, dur=dur, seed=i + m)
                elif config == '2L':
                    ts_noise_ET1[i] = noise_from_sqrt_psd(freq, ETD[i], fs=source_fs, dur=dur, seed=i + m)
            ...
            if fi == 0:
                nominal_chunk[local_idx] = netSNR
                event_nominal_snr = netSNR

            loss_denominator = loss_reference_nominal
            if loss_denominator is None:
                loss_denominator = event_nominal_snr

            if fi == 1:
                deci_chunk[local_idx]  = netSNR
                deci_loss_chunk[local_idx] = netSNR / loss_denominator

            if fi == 2:
                cinqui_chunk[local_idx]  = netSNR
                cinqui_loss_chunk[local_idx] = netSNR / loss_denominator

            if fi == 3:
                nova_chunk[local_idx] = netSNR
                nova_loss_chunk[local_idx] = netSNR / loss_denominator
            ...
            if cumulative:
                ...
                if netSNR >= 12:
                    time_to_m_SNR_chunk[local_idx] = time_to_SNR
                else:
                    time_to_m_SNR_chunk[local_idx] = time_to_SNR * -1

    return {
        'positions': np.array(event_positions, dtype=int),
        'nominal_SNR': nominal_chunk,
        'deci_SNR': deci_chunk,
        'cinqui_SNR': cinqui_chunk,
        'nova_SNR': nova_chunk,
        'deci_loss': deci_loss_chunk,
        'cinqui_loss': cinqui_loss_chunk,
        'nova_loss': nova_loss_chunk,
...
if __name__=="__main__":
    ...
    parser.add_argument('-nprocesses', help='number of worker processes to use; default = automatic', action='store', type=int)
    ...
    if nprocesses is not None and nprocesses < 1:
    ...
    nominal_SNR    = np.zeros(len(idx_tot))
    deci_SNR       = np.zeros(len(idx_tot))
    cinqui_SNR     = np.zeros(len(idx_tot))
    nova_SNR       = np.zeros(len(idx_tot))
    deci_loss      = np.zeros(len(idx_tot))
    cinqui_loss    = np.zeros(len(idx_tot))
    nova_loss      = np.zeros(len(idx_tot))
    ...
    def save_results():
        with open('../pkl/'+source_type+'_losses_'+case+'_'+source_type+'_sum_high_z_worstbest_test.pkl', 'wb') as file:
            pickle.dump((nominal_SNR, deci_SNR, cinqui_SNR, nova_SNR, deci_loss, cinqui_loss, nova_loss), file)

        with open('../pkl/'+source_type+'_times_to_merge_'+case+'_'+source_type+'_sum_high_z_worstbest_test.pkl', 'wb') as file:
            pickle.dump((time_tot, time_to_merge,time_to_m_SNR), file)

    def merge_chunk_results(chunk_result):
        positions = chunk_result['positions']
        nominal_SNR[positions] = chunk_result['nominal_SNR']
        deci_SNR[positions] = chunk_result['deci_SNR']
        cinqui_SNR[positions] = chunk_result['cinqui_SNR']
        nova_SNR[positions] = chunk_result['nova_SNR']
        deci_loss[positions] = chunk_result['deci_loss']
        cinqui_loss[positions] = chunk_result['cinqui_loss']
        nova_loss[positions] = chunk_result['nova_loss']
    ...
    def update_progress(processed_now):
        progress_state['completed'] += processed_now
    ...
    worker_state = {
        'idx_tot': np.array(idx_tot, dtype=int),
        'events': events,
        'source_type': source_type,
        'nIFOs': nIFOs,
        'IFOs': IFOs,
        'config': config,
        'coords': coords if 'coords' in locals() else None,
        'alt': alt if 'alt' in locals() else None,
        'armlegth': armlegth,
        'orient': orient if 'orient' in locals() else None,
        'file_list': file_list,
        'plots': plots,
        ...
        'loss_reference_nominal': None
    }

    init_process_worker(worker_state)

    if len(idx_tot) > 0:
        first_result = process_event_chunk([0])
        merge_chunk_results(first_result)
        worker_state['loss_reference_nominal'] = nominal_SNR[0]
        save_results()
        update_progress(1)

    pending_positions = np.arange(1, len(idx_tot), dtype=int)

    if len(pending_positions) > 0:
        auto_workers = (os.cpu_count() or 1) - 1 if (os.cpu_count() or 1) > 1 else 1
        requested_workers = auto_workers if nprocesses is None else nprocesses
        max_workers = max(1, min(requested_workers, len(pending_positions)))
    else:
        max_workers = 1
    ...
    if len(pending_positions) > 0:
        if max_workers <= 1:
            init_process_worker(worker_state)
            chunk_result = process_event_chunk(pending_positions.tolist())
            merge_chunk_results(chunk_result)
            save_results()
            update_progress(len(chunk_result['positions']))
        else:
            chunk_count = min(max_workers, len(pending_positions))
            event_chunks = [chunk.tolist() for chunk in np.array_split(pending_positions, chunk_count) if len(chunk) > 0]

            with ProcessPoolExecutor(max_workers=chunk_count, mp_context=mp.get_context("spawn"), initializer=init_process_worker, initargs=(worker_state,)) as executor:
                futures = [executor.submit(process_event_chunk, chunk) for chunk in event_chunks]
