...
def load_event_block(h5file, field_names, idx_block):
    """
    Legge un blocco di eventi dal file HDF5.
    Usa indici unici e ordinati per evitare problemi con fancy indexing di h5py.
    Restituisce array nello stesso ordine di idx_block.
    """
    idx_block = np.asarray(idx_block, dtype=np.int64)
    unique_idx, inverse = np.unique(idx_block, return_inverse=True)

    block_unique = {}
    for name in field_names:
        block_unique[name] = h5file[name][unique_idx]

    block = {}
    for name in field_names:
        block[name] = block_unique[name][inverse]
...
if __name__=="__main__":
    ...
    with h5py.File(source.catalogName, "r") as f:
        ...
        # Qui leggi solo quello che ti serve per costruire idx_tot
        N = f["tGPS"].shape[0]

        gps_min = int(to_gps("Jan 01 2030 00:00:00.000"))
        gps_max = int(to_gps("Jan 01 2031 00:00:00.000"))
        tGPS_all = np.array(unifom_dist(N, gps_min, gps_max))
        ...
        if source_type == "BNS":
            ...
            idx_tot = np.append(
                np.where(z_all <= 0.25)[0],
                np.array(idxs_rdn, dtype="int")[:, 0]
            )

            event_fields = [
            "dec", "ra", "psi", "Lambda1", "Lambda2",
            ...
            idx_tot = np.append(
                np.where((m1_all >= 100) & (m2_all < 100))[0],
                np.where((m2_all >= 100) & (m1_all < 100))[0]
            )
    ...
    loop_positions_sorted = np.argsort(idx_tot)
    n_chunks = math.ceil(len(loop_positions_sorted) / BLOCK_SIZE)

    for chunk_id in range(n_chunks):
        pos_chunk = loop_positions_sorted[chunk_id * BLOCK_SIZE : (chunk_id + 1) * BLOCK_SIZE]
        sel_chunk = idx_tot[pos_chunk]

        with h5py.File(source.catalogName, "r") as catalog_file:
            block = load_event_block(catalog_file, event_fields, sel_chunk)
        block["tGPS"] = tGPS_all[sel_chunk]
        
        for local_j,m in enumerate(pos_chunk):
            
            if local_j == 0: 
                avgT = 0
            else: avgT = elapsed/local_j
        
            progress_bar(m, len(idx_tot), 'Events evaluated '+str(local_j)+'/'+str(len(idx_tot)), avgT*(len(idx_tot)-local_j)/3600, 1, 70, '█')
            
            #print(m, events['tGPS'][sel], events['z'][sel])
    
            sel = sel_chunk[local_j]

            end_time        = block["tGPS"][local_j]
            declination     = block["dec"][local_j]
            right_ascension = block["ra"][local_j]
            polarization    = block["psi"][local_j]

            if source_type == "BNS":
                lambda1 = block["Lambda1"][local_j]
                lambda2 = block["Lambda2"][local_j]
            else:
                lambda1 = 0
                lambda2 = 0

            mass1 = block["m1_source"][local_j]
            mass2 = block["m2_source"][local_j]
            z     = block["z"][local_j]
            chi1z = block["chi1z"][local_j]
            chi2z = block["chi2z"][local_j]
            iota  = block["iota"][local_j]
            dL    = block["dL"][local_j]
            ...
            iidx_cut        = iidx[0] - int((freq_evolution.sample_times[-1] - freq_evolution.sample_times[idx_f_cut])*source.fs)
            iidx_st         = iidx[0] - int((freq_evolution.sample_times[-1] - freq_evolution.sample_times[idx_f_start])*source.fs)
            ...
            for fi, fl in enumerate(file_list[0:3]):
                ...
                for i in range(len(IFOs)):
                    ...
                    elif config == '2L':     ts_noise_ET1[i] = noise_from_sqrt_psd(freq, ETD[i], fs=source.fs, dur=dur, seed=i + m)
                ...
                if fi == 0:
                    ...
                    nominal_SNR[m] = netSNR
                    
                if fi == 1:
                
                    deci_SNR[m]  = netSNR
                    deci_loss[m] = netSNR/nominal_SNR[index]
                    
                if fi == 2:
                
                    cinqui_SNR[m]  = netSNR
                    cinqui_loss[m] = netSNR/nominal_SNR[index]
                    
                if fi == 3:
                
                    nova_SNR[m] = netSNR
                    nova_loss[m] = netSNR/nominal_SNR[index]
